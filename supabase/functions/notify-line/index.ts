...
function isHoliday(dateStr: string, events: EventRow[]): boolean {
    ...
    if (dayEvents.length > 0) {
        console.log(`[notify-line] isHoliday: イベント詳細=${JSON.stringify(dayEvents.slice(0, 3).map(e => ({ type: e.event_type, start: e.start_time })))}`)
    }

    // イベントがない日は休日
    if (dayEvents.length === 0) {
        console.log(`[notify-line] isHoliday: イベントなし → 休日`)
        return true
    }
    // 「休み」イベントがある場合は休日
    if (dayEvents.some(e => e.event_type === '休み')) {
        console.log(`[notify-line] isHoliday: 「休み」イベントあり → 休日`)
        return true
    }

    console.log(`[notify-line] isHoliday: 勤務イベントあり → 休日ではない`)
    return false
}
...
Deno.serve(async (req) => {
    ...
    console.log(`[notify-line] 実行開始: UTC=${now.toISOString()}, JST時刻=${currentJSTTime}, JST今日=${todayJST}, JST明日=${tomorrowJST}`)

    // 全ユーザーの設定を取得
    const { data: allSettings, error: settingsError } = await supabase
        .from('settings')
        .select('user_id, line_channel_access_token, line_user_id, notify_on_day_before, notify_day_before_time, notify_before_task, notify_before_task_minutes')
    ...
    for (const settings of (allSettings as SettingsRow[]) || []) {
        console.log(`[notify-line] ユーザー処理: ${settings.user_id}, token=${settings.line_channel_access_token ? 'あり' : 'なし'}, userId=${settings.line_user_id ? 'あり' : 'なし'}`)

        if (!settings.line_channel_access_token || !settings.line_user_id) {
            console.log('[notify-line] LINE設定なし、スキップ')
            continue
        }
        ...
        // 1) 前日通知のチェック
        console.log(`[notify-line] 前日通知チェック: enabled=${settings.notify_on_day_before}, 設定時刻=${settings.notify_day_before_time}, 現在時刻=${currentJSTTime}`)

        if (settings.notify_on_day_before && settings.notify_day_before_time === currentJSTTime) {
            console.log('[notify-line] 前日通知時刻一致！明日の休日チェック開始')
            ...
            console.log(`[notify-line] イベント取得: ${events?.length || 0}件`)

            // 明日の日付で休日判定（tomorrowJSTはYYYY-MM-DD形式）
            const isTomorrowHoliday = isHoliday(tomorrowJST, events as EventRow[] || [])
            console.log(`[notify-line] 明日(${tomorrowJST})は休日: ${isTomorrowHoliday}`)
            ...
            if (isTomorrowHoliday) {
                ...
                console.log(`[notify-line] タスク検索範囲: ${tomorrowStartUTC} ～ ${tomorrowEndUTC}`)
                ...
                console.log(`[notify-line] 明日のタスク: ${tasks?.length || 0}件, error=${tasksError?.message || 'なし'}`)
        ...
        // 2) タスク開始前通知のチェック
        console.log(`[notify-line] タスク通知チェック: enabled=${settings.notify_before_task}, 分前=${settings.notify_before_task_minutes}`)
        ...
        if (settings.notify_before_task && settings.notify_before_task_minutes >= 0) {
            ...
            console.log(`[notify-line] 対象時刻: ${targetJSTH}:${targetJSTM} (${settings.notify_before_task_minutes}分後)`)
            ...
            console.log(`[notify-line] 今日の未通知タスク: ${tasks?.length || 0}件`)
            ...
            for (const task of (tasks as ScheduledTaskRow[]) || []) {
                ...
                console.log(`[notify-line] タスク「${task.title}」: ${taskJSTH}:${taskJSTM}`)

                // 時間と分が一致するか確認
                if (taskJSTH === targetJSTH && taskJSTM === targetJSTM) {
                    console.log('[notify-line] 通知タイミング到来。送信権ロックを試行...')
                    ...
                    if (updatedTask && !updateError) {
                        ...
                        if (sent) {
                            ...
                            console.error('[notify-line] 送信失敗。notified_atは更新済みのままスキップ')
                            // 必要ならここでnotified_atをnullに戻す処理を入れるが、再送ループを防ぐためこのままにする
                        }
                    } else {
                        console.log('[notify-line] 既に他プロセスが処理済みのためスキップ')
                    }
                }
            }
        }
    ...
    console.log(`[notify-line] 完了: 前日通知=${notifiedCount.dayBefore}, タスク通知=${notifiedCount.taskReminder}`)
